...
class ChatServer2 {
  ...
  /**ConcurrentHashMap for keeping track of users who are online */
  static ConcurrentHashMap<String, ChatHistory> chatHistory = new ConcurrentHashMap<>(); 
  ...
  class ConnectionHandler implements Runnable { 
    ...
    private ArrayList<String> messageHistory = new ArrayList<String>(); //arrayList of the chat history with another user
    private ArrayList<Integer> userHistory = new ArrayList<Integer>(); //arrayList of which user sent which msg
    ...
    public void sendChatHistory(String key){
      ...
      if(chatHistory.get(key) == null){
        ...
        ChatHistory history = new ChatHistory(username, userTalkingTo);
        chatHistory.put(username + userTalkingTo, history); //create a chatHistory using both orders of username
        chatHistory.put(userTalkingTo + username, history);//Both users can access the same chat history

      }else{ //send chatHistory to current client
        
        messageHistory = chatHistory.get(key).getMessage();
        userHistory = chatHistory.get(key).getWhichUser();
        ...
        try{
          ...
          for(int i = 0; i < messageHistory.size(); i++){
            ...
            if(username.equals(chatHistory.get(key).getUser1())){ //if user is stored as user1 in chatHistory
              
              if(userHistory.get(i) == 1){
                
                output.println("\\s" + messageHistory.get(i));//send message indicating user sent it
                output.flush(); 

              }else if(userHistory.get(i) ==2){

                output.println("\\m" + messageHistory.get(i) +"\n\\u"+ userTalkingTo ); //send message indicating another user sent it
                output.flush();
              }
            }else if(username.equals(chatHistory.get(key).getUser2())){ //if user is stored as user2 in chatHistory
              
              if(userHistory.get(i) == 2){
                
                output.println("\\s" + messageHistory.get(i));//send message indicating user sent it
                output.flush();
                
              }else if(userHistory.get(i) == 1){

                output.println("\\m" + messageHistory.get(i) +"\n\\u"+ userTalkingTo ); //send message indicating another user sent it
                output.flush();
              }
            }else{
              System.out.println("There was an error with recognizing the messages of who it was sent from");
            }
    ...
    public void addToHistory(String message, String userFrom){
      ...
      chatHistory.get(username + userTalkingTo).add(userFrom, message);
    ...
    */
    ...
    public void run(){
      ...
      while(run){
        ...
        //System.out.println("message que is empty: " + messageQue.isEmpty());
        /**remove for final */
        System.out.print("");
